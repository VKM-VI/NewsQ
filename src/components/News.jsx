...
export class News extends Component {
    ...
    async componentDidMount(){
        ...
        let url =`https://newsapi.org/v2/top-headlines?country=${this.props.country}${this.props.category}&apiKey=85ef7428292c4fdf988546c463fa11e7&page=1`;
    ...
    HandleLeft = async ()=>{
        let url =`https://newsapi.org/v2/top-headlines?country=${this.props.country}${this.props.category}&apiKey=85ef7428292c4fdf988546c463fa11e7&page=${this.state.page - 1}`;
        this.setState({loading:true});
        let data=await fetch(url);
        let parseData=await data.json();
        console.log(parseData);
  ...
  render() {
    ...
    return (
      <div className='container my-3'>
        <h2 className={`text-center text-${mode==='light'?'black':'light'}`} style={{marginTop : '4rem'}}>{Heading}</h2>
